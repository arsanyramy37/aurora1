/* Aurora Animation Kit
   Modified: waits for language before initializing */
...
function initAuroraAnimations() {
  const SELECTORS = {
    ...
    cards:
      '.service-card, .portfolio-card, .contact-info-card, .contact-form-wrapper, .faq-item',
  ...
  const revealItems = [];
  query(SELECTORS.cards).forEach((card, index) =>
    revealItems.push({ element: card, index, kind: 'card' }),
  );
  query(SELECTORS.text).forEach((text, index) => {
    splitWords(text);
    revealItems.push({ element: text, index, kind: 'text' });
  });
  const footerEl = document.querySelector(SELECTORS.footer);
  if (footerEl)
    revealItems.push({ element: footerEl, index: 0, kind: 'footer' });
  ...
  function updateScrollEffects() {
    ...
    if (!reduced)
      parallax.forEach((element, index) => {
        ...
        element.style.setProperty(
          '--ak-parallax-y',
          `${Math.max(-18, Math.min(18, distance * (index % 2 ? 12 : -12)))}px`,
        );
  ...
  document.dispatchEvent(new CustomEvent('aurora:animready'));
}

// Wait for language on first load, otherwise init immediately
document.addEventListener('DOMContentLoaded', () => {
  if (document.documentElement.classList.contains('aurora-loading')) {
    document.addEventListener('aurora:langready', initAuroraAnimations, {
      once: true,
    });
  } else {
